...
void camera_process_keyboard(Camera* camera, int key, float deltaTime) {
    ...
    if (key == GLFW_KEY_W) glm_vec3_muladds(camera->front, velocity, camera->position);
    if (key == GLFW_KEY_S) glm_vec3_muladds(camera->front, -velocity, camera->position);
    if (key == GLFW_KEY_A) glm_vec3_muladds(right, -velocity, camera->position);
    if (key == GLFW_KEY_D) glm_vec3_muladds(right, velocity, camera->position);
}
